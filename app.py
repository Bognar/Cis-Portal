...
def main():
    ...
    if st.button("🚀 Generate Final Report", type="primary"):
        ...
        pdf.cell(0, 10, "Document Control", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.ln(5)

        # Header Styling
        pdf.set_fill_color(71, 91, 108) 
        pdf.set_text_color(255, 255, 255)  # <--- Sets text to white
        ...
        pdf.cell(70, 10, "Description", 1, 1, "C", True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)

        # Reset text color to black for the data rows
        pdf.set_text_color(0, 0, 0)        # <--- Sets text back to black
        ...
        for row in st.session_state.history:
            ...
            pdf.ln(10)
            pdf.set_font("helvetica", "B", 18)
            pdf.cell(0, 10, "Contact List", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.ln(5)

            # Header Styling
            pdf.set_fill_color(71, 91, 108)
            pdf.set_text_color(255, 255, 255) # <--- Set text to white
            pdf.set_font("helvetica", "B", 10)
            ...
            # Reset text color for the body of the table
            pdf.set_text_color(0, 0, 0)       # <--- Set text back to black
            pdf.set_font("helvetica", "", 10)
        ...
        # --- CIS Assessment Section ---
        pdf.add_page()
        pdf.set_font("helvetica", "B", 20)
        pdf.cell(0, 15, f"{section_num}. CIS Control Assessments", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        ...
        for ctrl, d_text in cis_info.items():
            ...
            # 1. Header Banner (Gold with White Text)
            pdf.set_fill_color(71, 91, 108)
            pdf.set_font("helvetica", "B", 12)
            pdf.set_text_color(255, 255, 255) # Professional White Text
            
            # Placeholder for header control Icon (if you have one, otherwise just text)
            # pdf.image("icons/asset.png", x=11, y=pdf.get_y()+2, w=6) 
            
            pdf.cell(0, 10, f"  {ctrl}", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)

            # 1. Move to the starting position
            pdf.set_x(10) 
            current_y = pdf.get_y()

            # 2. Draw the Background Bar FIRST (so the icon sits on top)
            pdf.set_fill_color(245, 245, 245)
            pdf.set_font("helvetica", "B", 10)
            # We use ln=0 to keep the "cursor" at the end of this cell for further elements
            pdf.set_text_color(0)
            pdf.cell(95, 8, f"        Client Score: {st.session_state.get(f'avg_{ctrl}',0)}/100", border="B", fill=True)

            # 3. Place the Icon (Overlaying the bar)
            # x=11 puts it just inside the 10mm margin
            # y calculation centers it vertically within the 8mm tall bar
            pdf.image("icons/score.png", x=12, y=current_y + 1.5, w=5)
            ...
            pdf.multi_cell(190, 5, f"Description: {clean_text(d_text)}", new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            
            # 4. Findings (Indented with Icon Placeholder)
            pdf.set_text_color(0)
            pdf.set_fill_color(245, 245, 245)
            pdf.ln(3)
            pdf.image("icons/finding.png", x=12, y=pdf.get_y(), w=5)
            pdf.set_x(17) # Create indent
            pdf.set_font("helvetica", "B", 10)
            pdf.cell(0, 6, "Findings:", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.ln(2) # This pushes the text down by 2 units (adjust as needed)
            pdf.set_font("helvetica", "", 10)
            pdf.set_x(10)
            pdf.write_html_style(clean_text(st.session_state.get(f"find_{ctrl}", "No findings.")), 10)
            # --- THE FIX ---
            
            pdf.ln(8)
            pdf.set_x(10) # Reset to your standard left margin (usually 10)

            # 5. Recommendations (Indented with Icon Placeholder)
            pdf.image("icons/check.png", x=12, y=pdf.get_y(), w=5)
            pdf.set_fill_color(245, 245, 245)
            pdf.set_x(17)
            pdf.set_font("helvetica", "B", 10)
            pdf.cell(0, 6, "Recommendations:", fill=True, new_x=XPos.LMARGIN, new_y=YPos.NEXT)
            pdf.ln(2) # This pushes the text down by 2 units (adjust as needed)
            pdf.set_font("helvetica", "", 10)
            pdf.set_x(10)
            pdf.write_html_style(clean_text(st.session_state.get(f"recom_{ctrl}", "No recommendations.")), 10)
            pdf.ln(12)
            # --- THE FIX ---
